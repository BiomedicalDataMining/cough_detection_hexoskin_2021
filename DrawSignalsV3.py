...
# ---------------------------------------------------------------------------------------------------------------------
def creat_dictionnay(SaveToFile=True):

    # List of all participants IDs
    ListPartcipants = ['221732', '221732']
    ...
    for ParticipentID in ListPartcipants:
        # Rawdata will be inside 'data/Raw_Dataset/ParticipentID', the ParticipentID, is the Hexoskin record Id
        RawFilePath = 'data/Raw_Dataset/' + ParticipentID

        # The events times will be inside the folder 'data/Tags/'
        ...
        # Substruct 4 Hours to get GMT-4 time (Same as the video time zone)
        ...
        # Get the start and the End time of the activity session
        ...
        # Get the start and the End time of the activity session in seconds
        ...
        offset = datetime.datetime.strptime(Tags.iloc[0]['VideoStartTime'] ,'%H.%M.%S.%f') - datetime.datetime.strptime(Tags.iloc[0]['HexoskinTime'] ,'%H.%M.%S.%f')

        # Fill the correct hexoskin time events using the video time
        for i in np.arange(3,Tags.shape[0]-3):
            Tags.loc[i,'HexoskinTime'] = (datetime.datetime.strptime(Tags.iloc[i]['VideoStartTime'] ,'%H.%M.%S.%f') - offset).time().strftime("%H.%M.%S.%f")

        # Concat 'record_specs'
        record_specs ={**record_specs , **{ParticipentID: {'offset_Hexoskin_Video': str(offset),
                                             'Hexoskin_start_date'  : Hexoskin_Start_Time.strftime('%Y-%m-%d %H:%M:%S.%f')}}
                      }

        # Concat 'data'
        data = {**data , **{ParticipentID: {'acceleration_X': dataAccX[Start_Aquisition * 64:End_Aquisition * 64],
        ...
        # Concat 'annotation'
        annotation = {**annotation, **{ParticipentID: Tags}}


    # Concat Dict into one 'datapkl_raw'
    record_specs = {'record_specs': record_specs}
    data         = {'data'        : data}
    annotation = {'annotation': annotation}
    datapkl_raw={**datatype_spec, **record_specs, **data, **annotation}

    # Save into zip pkl file
    if (SaveToFile):
